...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // The registration language decides the verification email's language
      // and becomes the account's initial preference.
        ...
        <div className="w-full max-w-md">
          {verificationSentTo ? (
            <div className="text-center">
              ...
              <h1 className="font-display text-2xl font-bold text-ink mb-3">{t('auth.checkEmailTitle')}</h1>
            ...
            <>
              <div className="text-center mb-8">
                <h1 className="font-display text-3xl font-bold text-ink mb-2">{t('auth.registerTitle')}</h1>
              ...
              {isGmail && !error && (
                <div className="flex items-start gap-2 p-3 rounded-xl bg-chocolate-50 border border-chocolate-100/40 mb-6">
                  ...
                  <div className="text-sm text-ink">
                    <p className="font-medium mb-1">{t('auth.gmailDetectedTitle')}</p>
                    <p className="text-ink-light text-xs leading-relaxed">
                      {t('auth.gmailDetectedBefore')} <button type="button" onClick={loginWithGoogle} className="font-semibold text-chocolate hover:text-chocolate-dark underline">{t('auth.continueWithGoogle')}</button> {t('auth.gmailDetectedAfter')}
                    </p>
                  </div>
                </div>
              )}
              ...
              <form onSubmit={handleSubmit} className="space-y-4 mb-6" noValidate>
                <div>
                  <label htmlFor="name" className="block text-sm font-medium text-ink mb-1.5">{t('auth.fullName')}</label>
                ...
                <div>
                  ...
                  <p className="text-[11px] text-ink-light/60 mt-1">{t('auth.passwordHint')}</p>
                </div>
                <div>
                  <label htmlFor="confirmPassword" className="block text-sm font-medium text-ink mb-1.5">{t('auth.confirmPassword')}</label>
                  <div className="relative">
                    <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-ink-light/60" />
                    <input
                      id="confirmPassword"
                ...
                >
                  {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : t('auth.createAccountButton')}
                </button>
              </form>
